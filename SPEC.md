...
> 最后更新：2026-08-15
> 本次变更：第四页「时讯」移除与首页重复的行情模块，新闻源切换为东方财富搜索接口，优化用户论坛身份展示
...
第四页聚焦「热点新闻 + 舆情 + 社区」，不重复首页已有的行情看板。

页面结构（由上到下）：

1. **时讯头部**：标题「市场时讯」+ 实时时钟 + 一句话定位（实时热点资讯 · 舆情解读 · 投资社区）。
2. **热点新闻**：
   - 数据来源：`GET /api/hot-news`（东方财富搜索接口）
   - 每条展示：情绪标签（恐慌 / 中性 / 狂热，仅辅助标签，不构成投资建议）、标题、来源（显示为"东方财富 · 媒体名"）、发布时间（相对时间）、热度分（🔥 hotScore）、关联板块标签、点击展开摘要 + 「查看原文 →」跳转东方财富原文。
   - 右上角显示数据来源「东方财富」+ 刷新按钮。
   - 空状态：显示「📭 暂无可用新闻数据，请稍后重试」+ 重新加载按钮，不白屏。
   - 不再展示「今日行情 / 指数卡片 / 涨跌幅」等与首页重复的行情模块。
3. **模拟社区**（本地 demo，与真实新闻明确区分）：
   - 每条帖子展示：用户头像（昵称/emoji + 稳定渐变色块）、昵称、标签（热议/技术/价值/求助/宏观/我）、时间、标题、正文、点赞数（本地 toggle）、评论数、分享按钮。
   - 内置演示帖子用于展示 UI，继续标注「演示数据」。
   - 用户可点击「发帖」发布本地帖子，帖子保存在 `localStorage`（key: `forum_user_posts`）。
   - 发帖作者身份使用 TPTI 人格名称 + 自定义用户名，格式为「{人格名} · {用户名}」：
     - TPTI 结果从 `localStorage.tradeti_state.result_type` 读取，人格配置来自 `TRADETI_PERSONALITIES`（名称、emoji、主色）
     - 自定义用户名首次发帖时输入，保存到 `localStorage.forum_username`，后续可在发帖弹窗中修改
     - 未测评 TPTI 时人格名显示「未测评型」；未设置用户名时基于 `client-id` 生成 `player_xxxx`
   - 新发布的帖子标记「我」并在列表中置顶于演示帖子之前，时间显示「刚刚」。
   - 点赞状态仅保存在当前浏览器会话（内存），不上传服务器。
   - 分享按钮保持 demo 占位，不产生真实网络请求。
4. **免责声明**：底部固定提示「以上信息仅供参考，不构成投资建议。市场有风险，投资需谨慎。新闻数据来自东方财富公开搜索，社区内容为演示数据。」
...
**数据源（唯一）**：东方财富搜索接口 `https://search-api-web.eastmoney.com/search/jsonp`（国内可访问、免费、无需 API Key），对应 AKShare `stock_news_em` 的底层网页数据。

**关键词策略**：服务端并行搜索多个财经关键词（A股、半导体、新能源、港股、美股），每个关键词取前 10 条，合并去重后返回 Top 30。支持 `?q=` 参数：若传入则只搜索该关键词（股票代码、行业词、市场词均可），不再并行多关键词。

**处理流程**：
1. 构造 JSONP 请求参数，`cb` 随机字符串以绕过浏览器 JSONP 限制，服务端剥离 JSONP 外壳后解析 JSON。
2. 提取字段：`title`、`content`（摘要）、`date`（发布时间）、`mediaName`（来源）、`url`、`code`（文章 ID）。
3. 本地处理：情绪识别（恐慌 / 中性 / 狂热，仅作为辅助内容标签，不构成投资建议）、关联板块、热度评分、标签提取。
4. 去重（URL + 文章 code + 标题前 20 字符）、按热度 + 时间排序、取 Top 30。
5. 支持 `?q=` 查询参数，直接透传到东方财富搜索关键词。
...
**降级策略**：所有关键词请求均失败或无数据时返回 `data: []`、`isFallback: true`、`message: "暂无可用新闻数据，请稍后重试"`，**不回退到任何国外源或 RSS 源，不使用 mock 数据**。

**前端 provider 映射**：`eastmoney-search` → `东方财富`。
...
| 2026-08-15 | 第四页「时讯」改版：移除与首页重复的「今日行情」模块；新闻源由不可用的新浪 RSS 切换为东方财富搜索接口（`eastmoney-search`），多关键词并行聚合 + 去重；用户论坛用户身份统一为「昵称首字 + 稳定渐变色块」头像，不再使用 emoji 占位 |
| 2026-08-15 | 第四页「时讯 · 模拟社区」支持本地发帖：作者身份格式为「{TPTI 人格名} · {自定义用户名}」，用户帖子存于 `localStorage.forum_user_posts` 并置顶；自定义用户名存于 `localStorage.forum_username`，未测评时人格名为「未测评型」，未设用户名时回退 `player_{client-id 短码}` |
