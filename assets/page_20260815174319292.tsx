...
type TechnicalGameProps = {
  onBack?: () => void;
  onComplete?: () => void;
};

type Mode = "learn" | "practice" | "exam" | "summary";
...
export function TechnicalGame({ onBack, onComplete }: TechnicalGameProps) {
      ...
      <div className="mx-auto w-full max-w-md">
        ...
        {mode === "exam" && (
          <section className="mt-3 rounded-[26px] border-2 border-[#394d79] bg-[#f6f7fb] p-4 text-[#1f2639] shadow-2xl">
            ...
            <div className="mt-3 space-y-2.5">{examQuestions[examIndex].options.map((option,idx) => (<button key={option} type="button" disabled={!!examFeedback} onClick={() => submitExam(idx)} className={`w-full min-h-[64px] rounded-[16px] border-2 px-4 py-3 text-left text-[13px] font-black leading-5 ${examChoice === idx ? "border-[#667fe0] bg-[#eef0ff]" : "border-[#d7ddec] bg-white"} disabled:cursor-default`}><span className="mr-2 text-[#667fe0]">{String.fromCharCode(65 + idx)}.</span>{option}</button>))}</div>
            {examFeedback && (<><div className="mt-4 rounded-[16px] border border-[#bfdcd8] bg-[#ecfaf8] px-4 py-3 text-[12px] font-bold leading-5 text-[#30716b]">{examFeedback}</div><button type="button" onClick={nextExam} className="mt-3 w-full rounded-[15px] bg-[#182344] py-3.5 text-[14px] font-black text-white">{examIndex === examQuestions.length - 1 ? "查看毕业总结 →" : "下一题 →"}</button></>)}
          </section>
        )}
...
export default function Page() {
  return (
    <TechnicalGame
      ...
      onComplete={() => { if (typeof window !== "undefined") window.alert("K 线学习已完成！"); }}
    />
  );
}
